                ...
                if confidence >=0.85 :#0.996:
                    ...
                    if  count>=10 and expression[-1] !=  sign:
                        ...
                        if sign == "fullstop":
                            var = expression
                            import language_tool_python
                            tool = language_tool_python.LanguageTool('en-US')  # English grammar correction
                            corrected_text = tool.correct(var)
                            translatedText = argostranslate.translate.translate(corrected_text, from_code, to_code)
                            add_input(f'{len(stored_inputs)} : {corrected_text}\nHindi : {translatedText}')
                            cls()
                            sign = ""
                        enter_text(sign)
